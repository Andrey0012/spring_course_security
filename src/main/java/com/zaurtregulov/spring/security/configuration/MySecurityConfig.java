...
//                withUser(userBuilder.username("Olga").password("Olga").roles("HR")).
//                withUser(userBuilder.username("Ivan").password("Ivan").roles("MANAGER", "HR"));
    }

    @Override
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests().antMatchers("/").hasAnyRole("HR", "EMPLOYEE","MANAGER").
                antMatchers("/hr_info").hasAnyRole("HR").
                antMatchers("/manager_info/**").hasAnyRole("MANAGER").and().formLogin().permitAll();
    }
}
